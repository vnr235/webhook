...
const WEBHOOK_URL = 'https://webhook-teal-nine.vercel.app/api/webhook';
...
dummyData.forEach(async (entry, index) => {
  try {
    const res = await axios.post(WEBHOOK_URL, entry, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
    console.log(`Sent entry ${index + 1}:`, res.data.message);
  } catch (err) {
    console.error(`Error sending entry ${index + 1}:`, err.message);
  }
});
